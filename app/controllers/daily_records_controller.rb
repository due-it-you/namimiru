class DailyRecordsController < ApplicationController
  ...
  def update
    ...
    if daily_record.update(daily_record_params)
      ...
      redirect_to user_daily_record_path(daily_record.user.uuid, daily_record.id), status: :see_other
    else
      flash[:alert] = "記録の更新に失敗しました。"
      redirect_to user_daily_record_path(daily_record.user.uuid, daily_record.id)
    end
  end
  ...
  def destroy
    ...
    if daily_record.destroy
      ...
      redirect_to user_daily_records_path(daily_record.user.uuid), status: :see_other
    else
