...
実体験として、私の母親が双極性障害を持っており、私と周囲の人は病状というセンシティブなことに関して聞くことが難しいゆえに、本人の実際の現在の症状と、周囲の人が考えている現在の症状に差異が発生してしまい、それが原因で大きなトラブルが起きてしまいました。
その経験から、記録を定期的に通知する機能も備えており、それによって相互での現状の認識を合わせていき、それに合わせた行動やコミュニケーションを手助けしてくれるアプリとなっています。
...
#### 認証機能
- [x] Emailでの新規登録機能
- [x] ソーシャルログイン（LINE, Google）でのログイン機能
- [x] パスワードの再登録機能
- [x] ゲストログイン機能
...
#### PWAによる機能
- [x] 固定時刻での日々の記録のリマインド通知機能（MVP段階では`gem whenever`を用いた固定の時刻でのリマインド機能）
- [x] 固定の頻度と時刻で、連携している当事者のユーザーがいる場合、グラフの画面を定期的にプッシュ通知機能（上記同様、固定の頻度と時刻）
